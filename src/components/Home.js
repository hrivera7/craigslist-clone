      ...
      {localStorage.getItem("token") ? (
            ...
            />
              {posts
                ? posts.map((post) => {
                    ...
                    <Card.Description className="delete-post">{post.location} {post.isAuthor&&<Icon onClick={() => deletePost(post._id)} name="trash" />}
                             
                            {post.isAuthor&&<Icon onClick={() => viewMessages(post._id)} name="eye" />} {!post.isAuthor && <Icon onClick={() => sendPost(post._id)} name="chat" /> }
                            
                            </Card.Description>
                            {post.price}
                            <div>{post.description.length < 100 ? post.description : null} </div>
                          </Card.Content>
                          <Card.Content >
                          </Card.Content>
                        </Card>
        ...
        <div className="flexbox home">
          ...
          <div className="search-bar"><Input
            ...
            <div className="post-listings">{posts
              ? posts.map((post) => {
                  return (
                    <div className="post" key={post._id}>
                      <Card>
                        <Card.Content style={{ border: "none" }}>
                          ...
                          <Card.Content className="post-bottom" >
                        </Card.Content>
                      </Card>
                    </div>
                  );
